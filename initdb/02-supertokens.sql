...
CREATE TABLE
    IF NOT EXISTS all_auth_recipe_users(
...
CREATE INDEX
    all_auth_recipe_users_pagination_index ON all_auth_recipe_users (time_joined DESC, user_id DESC);

CREATE TABLE
    IF NOT EXISTS userid_mapping(
...
CREATE TABLE
    IF NOT EXISTS dashboard_user_sessions(
...
CREATE TABLE
    IF NOT EXISTS emailpassword_users (
        user_id CHAR(36) NOT NULL,
...
CREATE TABLE
    IF NOT EXISTS emailpassword_pswd_reset_tokens (
        user_id CHAR(36) NOT NULL,
...
CREATE TABLE
    IF NOT EXISTS emailverification_verified_emails (
        user_id VARCHAR(128) NOT NULL,
        email VARCHAR(256),
...
CREATE TABLE
    IF NOT EXISTS emailverification_tokens (
        user_id VARCHAR(128) NOT NULL,
        email VARCHAR(256),
...
CREATE TABLE
    IF NOT EXISTS passwordless_users (
        user_id CHAR(36) NOT NULL,
...
CREATE TABLE
    IF NOT EXISTS passwordless_devices (
...
CREATE INDEX
    passwordless_devices_email_index ON passwordless_devices USING HASH (email);

CREATE INDEX
    passwordless_devices_phone_number_index ON passwordless_devices USING HASH (phone_number);

CREATE TABLE
    IF NOT EXISTS passwordless_codes (
        ...
        CONSTRAINT passwordless_codes_device_id_hash_fkey FOREIGN KEY (device_id_hash) REFERENCES passwordless_devices(device_id_hash) ON DELETE CASCADE ON UPDATE CASCADE
    );

CREATE INDEX
    passwordless_codes_created_at_index ON passwordless_codes(created_at);

CREATE INDEX
    IF NOT EXISTS passwordless_codes_device_id_hash_index ON passwordless_codes(device_id_hash);

CREATE TABLE
    IF NOT EXISTS jwt_signing_keys (
...
CREATE INDEX
    role_permissions_permission_index ON role_permissions(permission);
